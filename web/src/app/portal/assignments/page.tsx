...
export default async function PortalAssignmentsPage() {
  ...
  const { data: subs } = await supabase
    ...
    .select("assignment_id, status, grade, file_path")
    .eq("user_id", user.id);

  const subByAssign = new Map(
    (subs ?? []).map((s) => [
      s.assignment_id as string,
      s as { status: string; grade: string | null; file_path: string | null },
    ]),
  );
        ...
        {(assignments ?? []).length === 0 ? (
          ...
          (assignments ?? []).map((raw) => {
            ...
            return (
              <div
                key={a.id}
                className="card flex flex-wrap items-center gap-4 rounded-2xl px-6 py-5"
              >
                <div
                ...
                {sub?.grade ? (
                  <div className="display text-2xl font-black" style={{ color: "var(--green)" }}>
                    {sub.grade}
                  </div>
                ) : null}
                {status === "pending" ? (
                  <AssignmentSubmitForm assignmentId={a.id} userId={user.id} />
                ) : null}
              </div>
            );
          })
